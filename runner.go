...
func iperf(callNr int) {
	log.Println("iperf BW calculation started", callNr)
	defer log.Println("*****iperf ended*****")
	iperfArgs := fmt.Sprintf("iperf -c %s -p %d >> iperf_%d.txt", VOIP_SERVER, REMOTE_HOST_IPERF_PORT, callNr)
	fmt.Println(iperfArgs)
...
func fping(callNr int) {
	log.Println("fping RTT measurement started", callNr)
	defer log.Println("**********fping ended*********")
	fpingArgs := fmt.Sprintf("sudo -u mfrw fping -i 1000 -c 5 -eD %s > fping_%d.txt", VOIP_SERVER, callNr)
